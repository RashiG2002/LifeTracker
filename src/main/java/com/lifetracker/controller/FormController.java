...
public class FormController {
    ...
    public String addTask(
            ...
            @RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.TIME) LocalTime dueTime,
        ...
        task.setIsOutdoor(isOutdoor != null && isOutdoor);
        ...
        // Get weather alert for outdoor tasks
        if (task.getIsOutdoor() && location != null && !location.isEmpty()) {
            Map<String, Object> weather = weatherService.getWeatherByLocation(location);
            if (weather.containsKey("condition")) {
                task.setWeatherCondition((String) weather.get("condition"));
                Map<String, Object> alert = weatherService.getWeatherAlert(weather);
                
                if (alert.containsKey("hasSevereWarning") && (boolean) alert.get("hasSevereWarning")) {
                    redirectAttributes.addFlashAttribute("warning", 
                        "⚠️ SEVERE WEATHER WARNING: " + alert.get("message"));
                } else if (alert.containsKey("hasWarning") && (boolean) alert.get("hasWarning")) {
                    redirectAttributes.addFlashAttribute("warning", alert.get("message"));
                }
            }
        }
